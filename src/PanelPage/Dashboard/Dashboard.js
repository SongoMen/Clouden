...
export default class Dashboard extends Component {
  ...
  handleUploadSuccess = filename => {
    ...
    dbref.ref(`users/${uid}/info/disk`).update({
  ...
  componentDidMount = filename => {
    ...
    dbref.ref(`users/${uid}/info/spaceInBytes`).once(
      "value",
      function(snapshot) {
        this.setState({
          ...
          .on("value", function(snapshot) {
            ...
            snapshot.forEach(function(Snapshot) {
              ...
              firebase
                .storage()
                .ref(user)
                .child(snapshot.child(key).val())
                .getDownloadURL()
                .then(function(url) {
                  returnVal = url.slice(70, url.length);
                  content +=
                    "<a target = _blank href = https://firebasestorage.googleapis.com/v0/b/webapp-0021.appspot.com/o/" +
                    returnVal +
                    ' download = "' +
                    key +
                    '"><li>' +
  ...
  render() {
    const data = [
      ...
      {
        name: "Used Space",
        value: this.state.spaceInBytes * 2 - this.state.spaceInBytes,
      },
    ];
    var metadata = {
      customMetadata: {
            ...
            <div className="sections">
              <div className="sections__disk">
                <div className="sections__disk_heading">
                  ...
                  <p className="sections--paragraph">
                    {this.state.totalSize}/5.0
                  </p>
                </div>
                <PieChart
                  width={350}
